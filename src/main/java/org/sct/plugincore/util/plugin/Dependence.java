...
import org.sct.plugincore.data.DependenceData;
...
public class Dependence {
    ...
    static {
        ...
        depenDir = new File(PluginCore.getInstance().getDataFolder() + File.separator + "Dependencies");
    }
    ...
    public static void download(Dependence dependence) {
        pool.submit(() -> {
            ...
            File dependenceFile = new File(PluginCore.getInstance().getDataFolder() + File.separator + "Dependencies" + File.separator + dependence.getFileName());
    ...
    private static void sendLoadMessage(String dependency) {
        sendMessage(String.format("§7[§ePluginCore§7]§bDependency: %s §2load successfully!", dependency));
    }
