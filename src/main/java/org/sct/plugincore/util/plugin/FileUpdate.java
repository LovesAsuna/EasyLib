...
public class FileUpdate {
    public static void update(JavaPlugin instance, String fileName, String path) {
        try {
            ...
            Bukkit.getScheduler().runTaskLaterAsynchronously(PluginCore.getInstance(), () -> {
                deleteFile(originFile, savePath);
            }, 20L);
        } catch (IOException e) {
            StackTrace.printStackTrace(e);
            Bukkit.getConsoleSender().sendMessage("§7[§ePluginCore§7]§c自动更新配置文件时出错!");
        }
    }

    private static boolean isSection(String path) {
        return PluginCore.getInstance().getConfig().getConfigurationSection(path) == null ? false : true;
    }
